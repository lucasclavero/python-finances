...
Variabilní symbol: %s""" % (self.provider.bank_name, self.provider.bank_account, self.vs),
    ...
    def drawItems(self, TOP, LEFT):
        ...
        for x in self.items:
            ...
            self.pdf.drawString((LEFT+122)*mm, (TOP-i)*mm, "%.2f,- USD" % x.price)
            self.pdf.drawString((LEFT+150)*mm, (TOP-i)*mm, "%.2f,- USD" % (x.total()))
            i += 5
            total += x.total()
        ...
        self.pdf.setFont("DejaVu", 12)
        self.pdf.drawString((LEFT+130)*mm, (TOP-i-10)*mm, "Celkem: %d ,- USD" % total)
